...
We also developed a Bayesian model that accurately predicts the severity of next year's influenza season given the data from the current year. We found this model to also perform well, with demonstration on the European 2018 cycle, with all of the values falling within our High Posterior Density interval.
