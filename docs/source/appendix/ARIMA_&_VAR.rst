...
After the above operations, we notice that :math:`p \in \{0,1,...,20\}` is the technically feasible set of lags, thus regress it and obtain the SBIC. We found VAR(9) to be the best, which gets an MSE to be 5927. Using the same procedure, run all AR(p) up to 80, then BIC suggests AR(3) to be the best in the univariate world. Surprisingly, that univariate model yields an MSE=4968.

Such surprising result might be reasonsed by the following plot:
